...
const INITIAL_DOCTORS = [
  { id: 1, name: 'Dr. Sarah Johnson', specialty: 'Cardiology',  exp: 4,  degree: 'MBBS, MD', status: 'active',   patients: 28, rating: 4.8, email: 'sarah.j@hospital.com', phone: '+1 234-567-8901', bio: 'Experienced cardiologist with 4 years of clinical practice.' },
  { id: 2, name: 'Dr. Aisha Khan',    specialty: 'Radiology',   exp: 10, degree: 'MBBS, MS', status: 'active',   patients: 41, rating: 4.9, email: 'aisha.k@hospital.com', phone: '+1 234-567-8902', bio: 'Senior radiologist specializing in MRI and CT scans.' },
  { id: 3, name: 'Dr. Michael Chen',  specialty: 'Neurology',   exp: 7,  degree: 'MBBS, DM', status: 'inactive', patients: 0,  rating: 4.6, email: 'michael.c@hospital.com', phone: '+1 234-567-8903', bio: 'Currently on leave. Specializes in neurodegenerative diseases.' },
  { id: 4, name: 'Dr. Priya Patel',   specialty: 'Pediatrics',  exp: 5,  degree: 'MBBS, MD', status: 'active',   patients: 33, rating: 4.7, email: 'priya.p@hospital.com', phone: '+1 234-567-8904', bio: 'Compassionate pediatrician dedicated to child health.' },
];

const SPECIALTIES = ['All', 'Cardiology', 'Neurology', 'Radiology', 'Pediatrics', 'Orthopedics', 'Pathology', 'General'];
...
export default function DoctorsTab() {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this doctor?")) {
      ...
      if (view === 'profile' && selectedDoctor?.id === id) {
        setView('list');
  ...
  const handleSave = (doctorData) => {
    if (selectedDoctor && selectedDoctor.id === doctorData.id) {
      // Update
      setDoctors(d => d.map(x => x.id === doctorData.id ? doctorData : x));
    } else {
      // Add
      setDoctors(d => [...d, doctorData]);
    }
    setView('list');
    setSelectedDoctor(null);
  };
  ...
  if (view === 'form') {
    return (
      <DoctorForm 
        ...
        onSave={handleSave} 
        onCancel={() => {
          setView(selectedDoctor && view === 'form' && selectedDoctor.id ? 'profile' : 'list');
          if (!selectedDoctor || !selectedDoctor.id) setSelectedDoctor(null);
        }} 
      />
    );
  }
  ...
  if (view === 'profile') {
    return (
      <DoctorProfile 
        doctor={selectedDoctor} 
        onBack={() => { setView('list'); setSelectedDoctor(null); }}
        onEdit={openEditForm}
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Cards grid */}
      {filtered.length === 0 ? (
        <div className="py-16 text-center text-sm text-slate-400 bg-white rounded-sm border border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((doc) => {
            ...
            return (
              <div
                key={doc.id}
                className="bg-white border border-slate-200 rounded-sm p-5 hover:shadow-md hover:border-slate-300 transition-all group cursor-pointer"
                onClick={() => openProfile(doc)}
              >
                {/* Top row */}
                <div className="flex items-start gap-4">
                  {/* Avatar */}
                  <div className={`w-12 h-12 rounded-sm flex items-center justify-center text-sm font-bold shrink-0 overflow-hidden ${spec.avatar}`}>
                    {doc.image ? (
                  ...
                  <div className="flex items-center gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                    <button onClick={() => openEditForm(doc)} className="w-7 h-7 flex items-center justify-center rounded-sm text-slate-400 hover:text-amber-600 hover:bg-amber-50 transition-all">
                      <Edit className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => handleDelete(doc.id)} className="w-7 h-7 flex items-center justify-center rounded-sm text-slate-400 hover:text-red-600 hover:bg-red-50 transition-all">
